...
- **Scope**: This codebase generates, signs, and publishes multi-component `platform.json` release manifests, manages the atomic `latest` pointer, and maintains the POSIX cross-platform installer template and build rail (`install.sh.in` -> `install.sh`). The canonical installer is published at `https://solstone.app/install.sh`; `https://solstone.app/platform-install.sh` remains a byte-identical compatibility URL. On macOS it acquires only the native journal and solstone apps, never a standalone Journal runtime.
- **Source of Truth**:
