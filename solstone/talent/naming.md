...
1. **Thickness gate** — Run `sol call sol thickness`. If `ready` is `false`, exit silently.
2. **Name gate** — Run `sol call sol name`. If `name_status` is not `"default"`, exit silently.
...
3. `sol call sol name` — current name and status (confirms default)
4. `sol call sol thickness` — the thickness signals (confirms readiness)
...
Then:
- **Accept**: Run `sol call sol set-name "NAME" --status self-named`
...
Do this by running `sol call sol set-name` with the current name and status, plus updating these fields via the agent config mechanism.

## Proposal Cap

If `proposal_count` from `sol call sol name` is 3 or more, do NOT propose. Instead say:
...
If `last_proposal_date` from `sol call sol name` is within the last 14 days, exit silently. Do not re-propose.
