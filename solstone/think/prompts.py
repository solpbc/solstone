...
# Cached raw template content loaded from solstone/think/templates/*.md
...
def _load_raw_templates() -> dict[str, str]:
    """Load raw template files from solstone/think/templates/ directory.
...
def _load_templates(template_vars: dict[str, str] | None = None) -> dict[str, str]:
    """Load and substitute template files from solstone/think/templates/ directory.
...
def _resolve_facets(facet: str | None) -> str:
    ...
    if facet:
        try:
            from solstone.think.facets import facet_summary

            return f"## Facet Focus\n{facet_summary(facet)}"
        except Exception:
            return ""
    try:
        from solstone.think.facets import facet_summaries
...
def load_prompt(
    ...
    *,
    ...
    - Templates from solstone/think/templates/*.md:
