...
def _variant_for_platform(os_name: str, arch: str) -> str | None:
...
def _verify_mac_cache(cache_dir: Path) -> bool:
    return all(
        (cache_dir / relative_path).is_file() for relative_path in MAC_MODEL_FILES
    )
...
def _run_mac_helper(cache_dir: Path) -> dict[str, Any]:
    ...
    if not helper_path.is_file() or not os.access(helper_path, os.X_OK):
        raise RuntimeError(
            f"parakeet install failed: helper not found or not executable at {helper_path}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install or verify the Parakeet model cache."
    )
    mode_group = parser.add_mutually_exclusive_group()
    mode_group.add_argument(
        "--check",
        action="store_true",
        help="Verify the sentinel/cache, fetching the model if needed (default behavior).",
    )
    mode_group.add_argument(
        "--force",
        action="store_true",
