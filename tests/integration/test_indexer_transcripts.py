...
def test_transcripts_indexer_scan_and_search():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert (
                scan_count > 0
            ), f"Expected to scan transcript files, got {scan_count}"
...
def test_transcripts_indexer_by_type():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            audio_count = sum(
                1 for r in audio_results if r["metadata"]["type"] == "audio"
            )
            ...
            screen_count = sum(
                1 for r in screen_results if r["metadata"]["type"] == "screen"
            )
