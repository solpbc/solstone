...
def test_entities_indexer_scan_and_search():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert len(results) > 0, f"Should have actual results for 'John Doe', got {len(results)} results"
            
            # Debug output
            if results:
                print(f"Found {len(results)} results for 'John Doe':")
                for i, r in enumerate(results):
                    print(f"  Result {i}: text='{r.get('text', 'NO TEXT')}', metadata={r.get('metadata', {})}")
            
            found_john = False
            for result in results:
                # More flexible matching
                text = result.get("text", "").lower()
                if "john" in text or result.get("metadata", {}).get("name", "").lower() == "john doe":
                    found_john = True
