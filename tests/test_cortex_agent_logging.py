...
def test_json_event_writer():
    ...
    try:
        ...
        assert Path(test_file).exists(), "JSONEventWriter should create file when path provided"
        ...
        with tempfile.TemporaryDirectory() as journal_tmp:
            ...
            try:
                ...
                if journal_agents.exists():
                    ...
                    assert len(files) == 0, f"JournalEventWriter should not create files, found: {files}"
            finally:
                if env_backup:
