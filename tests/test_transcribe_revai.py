...
class TestConvertRevaiToSolstone:
    """Tests for the legacy convert_revai_to_solstone function."""

    def test_per_speaker_mode(self):
        """per_speaker=True produces one entry per monologue."""
        ...
        # Per-speaker mode: one entry for the whole monologue
        result = convert_revai_to_solstone(revai_json, per_speaker=True)
        assert len(result) == 1
        assert result[0]["text"] == "First. Second."
        assert result[0]["speaker"] == 1

    def test_per_sentence_mode(self):
        """per_speaker=False (default) splits on sentence boundaries."""
        from observe.revai import convert_revai_to_solstone
        ...
        # Per-sentence mode: two entries (one per sentence)
        result = convert_revai_to_solstone(revai_json, per_speaker=False)
        assert len(result) == 2
    ...
    def test_timestamp_formatting(self):
        ...
        assert result[0]["start"] == "01:01:01"

    def test_confidence_descriptions(self):
        """Low/high confidence adds description field."""
        from observe.revai import convert_revai_to_solstone
        ...
        result = convert_revai_to_solstone(low_conf, per_speaker=True)
        assert result[0].get("description") == "low confidence"
        ...
        result = convert_revai_to_solstone(high_conf, per_speaker=True)
        assert result[0].get("description") == "clear"

    def test_source_field(self):
        """Entries have source='import' field."""
        from observe.revai import convert_revai_to_solstone
            ...
            "monologues": [
                {
                    "speaker": 0,
                    "elements": [
                        {"type": "text", "value": "Test", "ts": 0.0, "end_ts": 0.5}
                    ],
                }
            ]
        }
