...
"""Pre-hook: inject conversation memory into unified muse context.

Loaded via hook config: {"hook": {"pre": "conversation_memory"}}

Replaces CONVERSATION_MEMORY_INJECTION_POINT in the unified muse's
...
def pre_process(context: dict) -> dict | None:
    """Inject conversation memory into the unified muse's user instruction.
