...
from observe.transcribe.overlap import compute_overlap_and_logprobs
...
def process_audio(
    ...
    try:
        ...
        overlap_fraction_value, _pyannote_logprobs = compute_overlap_and_logprobs(
            audio_buffer
        )
        ...
        # Skip when overlap is near zero — the recording is effectively solo speech
        # and diarization adds no value.  Otherwise pass the pyannote log-probs
        # computed above so the diarizer skips its own (duplicate) pyannote pass.
        ...
        if resolved_backend in {"parakeet", "whisper"}:
            if overlap_fraction_value < _DIARIZE_MIN_OVERLAP:
                ...
                try:
                    from observe.transcribe.diarize import diarize_auto_k

                    labels = diarize_auto_k(
                        raw_path,
                        statements,
                        avg_log_probs=_pyannote_logprobs,
                        audio=audio_buffer,
                    )
                    assigned = 0
        ...
        # Update cross-segment speaker candidate pool (best-effort; never blocks)
        if embeddings_data:
            try:
                ...
                tracker.load()
                ct_result = tracker.process_segment(
                ...
                if ct_result.get("status") == "ok":
                    logging.info(
                        "Speaker candidates: %d new, %d merged, %d unstable (pool=%d)",
                        ct_result["n_new"],
                        ct_result["n_merges"],
                        ct_result["n_unstable"],
                        ct_result["total_candidates"],
                    )
            except Exception:
