...
def show_prompt_context(
    ...
    *,
    ...
    Builds config and pipes to `python -m think.talents --dry-run` to show exactly
    ...
    # Activity-scheduled talents need --facet and --activity
    ...
    if schedule == "activity":
        ...
        try:
            talent_config = get_talent(name, facet=facet)
            config.update(talent_config)
    ...
    # Run think.talents --dry-run
    config_json = json.dumps(config)
    try:
        result = subprocess.run(
            [sys.executable, "-m", "think.talents", "--dry-run"],
            input=config_json + "\n",
            capture_output=True,
            text=True,
            timeout=30,
