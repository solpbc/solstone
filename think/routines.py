...
def save_config(config: dict[str, dict[str, Any]]) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=routines_dir, suffix=".tmp", prefix=".config_"
    )
...
def check() -> None:
    ...
    for routine in _config.values():
        ...
        try:
            ...
            logger.warning("Routine %s has invalid timezone %r, skipping", routine_id, tz)
            continue
        minute_key = local_now.strftime("%Y-%m-%d %H:%M")
        if _last_fired.get(routine_id) == minute_key:
            continue

        if cron_matches(routine["cadence"], local_now):
