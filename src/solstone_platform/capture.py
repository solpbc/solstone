...
class _SourceCapturer:
    ...
    def _safe_copy_file(
        ...
        try:
            ...
            # Read nonzero prefix first if file is non-empty
            chunks: list[bytes] = []
        ...
        # Enforce size limits by name
        max_allowed = _classify_max_size_by_name(name)
        if len(body) > max_allowed:
            raise Refusal(
                CAPTURE_SIZE_LIMIT_EXCEEDED,
                f"file {rel_path} size {len(body)} exceeds limit {max_allowed}",
            )
        ...
        if self.total_bytes > MAX_AGGREGATE_BYTES:
            raise Refusal(
                CAPTURE_SIZE_LIMIT_EXCEEDED,
                f"aggregate capture size {self.total_bytes} exceeds limit {MAX_AGGREGATE_BYTES}",
            )
